...
Solenoid.SWG.Form = function (element, inputNameSeparator, invalidInputClass)
{
    ...
    public.reset = function ()
    {
        ...
        // (Iterating each entry)
        public.element.querySelectorAll('.input.input-invalid').forEach
        (
            function (inputElement)
            {
    ...
    public.readFile = async function (file)
    {
        // Returning the value
        return new Promise
        (
    ...
    // Returns [Promise:object]
    public.getInput = async function (mode)
    {
        // Returning the value
        return new Promise
        (
            async function (resolve, reject)
            {
                ...
                for (let inputElement of public.element.querySelectorAll('.input:not(.input-ignore)'))
                {// Processing each entry
                    // (Setting the value)
                    let inputValue = null;
                    ...
                    switch ( inputElement.getAttribute('type') )
                    {
                        ...
                        case 'file':
                            ...
                            {// Processing each entry
                                // (Appending the value)
                                inputValue.push( await public.readFile( file ) )
                            }
    ...
    // Returns [void]
    public.setInput = function (input)
    {
        // (Getting the value)
        const kvData = private.objectCompress( input, public.inputNameSeparator );

        for (const k in kvData)
        {// Processing each entry
            // (Getting the element)
            const inputElement = public.element.querySelector( `.input[name="${ k }"]` );
